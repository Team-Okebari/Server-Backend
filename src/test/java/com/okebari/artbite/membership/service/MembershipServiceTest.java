...
class MembershipServiceTest {
	...
	@Test
	@DisplayName("멤버십 등록 성공 - 첫 등록")
	void enrollMembership_success_firstEnrollment() {
		// Given
		...
		when(membershipRepository.save(any(Membership.class))).thenAnswer(invocation -> {
			Membership savedMembership = invocation.getArgument(0);
			ReflectionTestUtils.setField(savedMembership, "id", 1L); // Mock ID for saved membership
			return savedMembership;
		});

		// When
		MembershipStatusResponseDto result = membershipService.enrollMembership(testUser.getId(), requestDto);
	...
	@Test
	@DisplayName("멤버십 등록 실패 - 이미 활성 멤버십 존재")
	void enrollMembership_fail_alreadyActiveMembership() {
		// Given
		EnrollMembershipRequestDto requestDto = new EnrollMembershipRequestDto();
		requestDto.setAutoRenew(true);
		...
		BusinessException exception = assertThrows(BusinessException.class,
			() -> membershipService.enrollMembership(testUser.getId(), requestDto));
		assertThat(exception.getErrorCode()).isEqualTo(ErrorCode.MEMBERSHIP_ALREADY_ACTIVE);
